...
if run_button:
    if not tickers:
        ...
        # ========== ADDED: Ticker Filter Dropdown ==========
        # Get unique tickers and create dropdown options
        all_tickers_in_results = sorted(list(set([r["ticker"] for r in detail_rows])))
        ticker_filter_options = ["All Tickers"] + all_tickers_in_results
        
        # Initialize session state for ticker filter if not exists
        if "selected_ticker_filter" not in st.session_state:
            st.session_state.selected_ticker_filter = "All Tickers"
        
        # Create dropdown - use on_change to update session state when changed
        def update_ticker_filter():
            st.session_state.selected_ticker_filter = st.session_state.ticker_dropdown
        
        selected_ticker_filter = st.selectbox(
            "Filter by ticker:",
            options=ticker_filter_options,
            index=ticker_filter_options.index(st.session_state.selected_ticker_filter) if st.session_state.selected_ticker_filter in ticker_filter_options else 0,
            key="ticker_dropdown",
            on_change=update_ticker_filter
        )
        
        # Filter the detail rows based on selection
        if selected_ticker_filter != "All Tickers":
            detail_rows = [r for r in detail_rows if r["ticker"] == selected_ticker_filter]
        # ===================================================
